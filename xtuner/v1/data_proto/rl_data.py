...
class Status(Enum):
    ...
    EXPIRED = "expired"
    FAILED = "failed"
    # 归档，这个状态还是要保留，用不用再说，用于表示这个数据已经不参与训练了，但保留在数据库里以备查询
    ARCHIVED = "archived"



class MultimodalInfo(TypedDict):
    # 使用TypedDict给出pixel_values的类型提示，但在实际使用中，考虑到性能和内存占用，可能会使用Ray的ObjectRef来传递大规模的tensor数据
...
class RolloutState(BaseModel):
    ...
    response_ids: list[int] | None = None  # 每一次实际输出，覆盖写
