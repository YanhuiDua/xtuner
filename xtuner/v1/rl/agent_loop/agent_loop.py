...
class AgentLoop(ABC):
    ...
    async def collect_rollout_group(
        ...
        **kwargs,
        ...
        if self.is_valid_sample_fn is None and self.teacher_clients:
        ...
        if get_group_status(group) != Status.COMPLETED or not self.teacher_clients:
            return group
        teacher = route_rollout_teacher_client(
            group[0],
            data_source_teacher_map=self.data_source_teacher_map,
        ...
        return list(await asyncio.gather(*(create_task(teacher.compute_logprobs(state)) for state in group)))
