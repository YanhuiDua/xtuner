...
class DataFlowConfig(BaseModel):
    ...
    **Examples:**
    ...
    over_generation_ratio: Annotated[
        float,
        Parameter(help="Ratio to over-generate samples."),
    ] = 1.0
...
class DataFlow:
    ...
    async def concurrent_task_runner(self):
        ...
        """
        waiting_tasks = set()
        with tqdm(total=self.updated_batch_size, desc="rollout_controller for training samples") as pbar:
            update_step = max(1, int(self.updated_batch_size * 0.01))
            next_update_threshold = update_step
            while (
                self.finished_samples_count < self.updated_batch_size
                and self.failed_samples_count < self.updated_batch_size
                and self.skipped_sample_count < self.updated_batch_size
            ):
                ...
                while len(waiting_tasks) < self.config.max_concurrent:
                    ...
                    if (
                        ...
                        and self.finished_samples_count + len(waiting_tasks) >= self.updated_batch_size
                    ):
                        break
        ...
        if self.finished_samples_count >= self.updated_batch_size:
            self.logger.info("Target batch size reached. Pausing env controller.")
        if self.failed_samples_count >= self.updated_batch_size or self.skipped_sample_count >= self.updated_batch_size:
    ...
    async def run(
        ...
        # reset global batch size 
        self.updated_batch_size = self.target_batch_size * self.config.over_generation_ratio
